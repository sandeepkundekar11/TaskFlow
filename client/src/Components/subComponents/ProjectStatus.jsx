...
const ProjectStatus = () => {
  ...
  const { callApi: getProjectInfo, data: projectData } = useGetApi(`${BASE_URL}/user/projectAndCompletedSprint/${projectId}`)
  useEffect(() => {
  ...
  const { callApi: addSubTask, data: addSubTaskMessage, loading: addSubtaskLoading } = usePostApi(`${BASE_URL}/user/createSubTask/${projectId}/${selectedTaskId}`)
  ...
  useEffect(() => {
    ...
    setCurrentSprintInfo((prev) => updateCurrentSprintAndAddSubTask(prev, selectedTaskId, newSubTask))
      ...
      <div className="border-b mt-3 pb-4">
        {
          ProjectInfo?.sprint?.map((data, index) => {
            return (
              <div className="h-14 p-2 border rounded-sm w-full grid grid-cols-3  items-center" key={index}>
                <h1 className="text-lg font-bold col-span-1">{data?.name}</h1>
                {/*  */}
                <div className="col-span-1 flex items-center h-full">
                  <AvatarGroup users={ProjectInfo?.projectInfo?.users?.map((user) => user.name)} />
                </div>
                {/*  */}
                <div className="col-span-1 flex justify-end">
                  <Button variant="outline" className=" font-semibold text-gray-600 w-28 text-center rounded-xl hover:bg-white cursor-not-allowed">Completed</Button>
                </div>
              </div>
            )
          })
